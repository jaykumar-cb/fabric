...
func (vdb *VersionedDB) retrieveMetadata(nsKeysMap map[string][]gocb.BulkOp) (map[string][]*couchbaseDoc, error) {
	...
	executionResults := make(map[string][]*couchbaseDoc)
...
func retrieveNsMetadata(db *couchbaseDatabase, keys []gocb.BulkOp) ([]*couchbaseDoc, error) {
	...
	// accumulate results from each batch
	var executionResults []*couchbaseDoc
...
func (b *subNsMetadataRetriever) execute() error {
	var err error
	if b.executionResult, err = b.db.batchGetDocument(b.keys); err != nil {
		return err
	}
	return nil
}
